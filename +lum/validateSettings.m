...
if S.Session.UseOpto && budget < 1
    fail('noTimersForLight', ...
         ['No global timers are left for light: the hold window takes %d, sync %d, the '...
          'hold clock %d, the light clock %d and timed stimulus components %d of this '...
          'machine''s %d.'], ...
         reserved.HoldWindow, reserved.Sync, reserved.HoldClock, reserved.LightClock, ...
         reserved.Components, rig.Limits.GlobalTimers);
end
...
if lum.HoldShaping.growsHold(S)
    ...
    if S.GUI.HoldTarget < fullHold
        notes{end+1} = sprintf(['The target hold (%g s) is shorter than the stimulus '...
                                'window plus the post-stimulus hold (%g s): the hold never '...
                                'covers the whole stimulus.'], S.GUI.HoldTarget, fullHold);
    end
end
...
if lum.HoldShaping.growsHold(S)
    ...
    firstHold = lum.HoldShaping.fullHold(S);
end
