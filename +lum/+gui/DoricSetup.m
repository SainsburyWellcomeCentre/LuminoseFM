classdef DoricSetup < handle
    ...
    properties (Access = private)
        ...
        calibrationFolder = ''
        calibrationWindow = []
        calibrationCurrents = lum.gui.DoricCalibration.DefaultCurrentsmA
    ...
    methods
        function obj = DoricSetup(parent, S, t, onEdit, led, varargin)
            ...
            if isfield(S.Doric, 'CalibrationCurrentsmA')
                obj.calibrationCurrents = S.Doric.CalibrationCurrentsmA;
            end
            obj.build(parent, S);
    ...
    methods (Access = private)
        function build(obj, parent, S)
            ...
            obj.Controls.Calibrate = uibutton(form, 'Text', ['Calibrate LED power' char(8230)], ...
                ...
                'Tooltip', ['Measure the power leaving the cables with a power meter, two at a time: '...
                            ...
                            '0 to 1000 mA in 100 mA steps, never above the channel''s limit. Readings '...
                            'already saved are shown, to be added to. Each is the calibration of that '...
                            'cable on that channel; saving replaces the earlier one.']);
            ...
            for k = 1:2
                ...
                obj.Controls.MaxCurrent(k) = uieditfield(form, 'numeric', 'Value', S.Doric.MaxCurrentmA(k), ...
                    ...
                    'Tooltip', ['The highest current this channel may be set to, in mA. A request above it '...
                                'is refused. At most 1000 mA, the LED''s rating (the default). Doric '...
                                'recommends 700 for light held on for long; the driver''s front knob '...
                                'must allow as much.']);
                uilabel(form, 'Text', '');
        ...
        function calibrate(obj)
            ...
            obj.calibrationWindow = lum.gui.DoricCalibration(c.Bundle.Value, {c.CableA.Value, c.CableB.Value}, ...
                ...
                'CurrentsmA', obj.calibrationCurrents, ...
